...
const ProfileSettingMentee = () => {
  ...
  const updateprofile = (e) => {
    ...
    if (
      ...
      UpdateStudent(fd, id, history)
    } else {
      notify({
        text: "Ma'lumotlaringizni to'liq kiriting.",
      })
      setErrors(
        Validation({
          first_name,
  ...
  useEffect(() => {
    ...
    setTelegram(
      userdata?.user?.telegram_number ? userdata?.user?.telegram_number : '',
    )
    setBirthDate(userdata?.user?.birth_date ? userdata?.user?.birth_date : '')
    setRegion(userdata?.user?.region ? userdata?.user?.region : '')
    setCountry(userdata?.user?.country ? userdata?.user?.country : '')
    setAboutUs(userdata?.user?.about_us ? userdata?.user?.about_us : 'Telegram')
    setJob(userdata?.user?.job ? userdata?.user?.job : 'Talaba')
    setExperience(
      userdata?.user?.experience ? userdata?.user?.experience : "Boshlang'ich",
    )
    setTarget(userdata?.user?.target ? userdata?.user?.target : '')
    return () => {
      setImgModal(false)
    }
  }, [image, userdata])
      ...
      {register && (
        <div className="alert_content">
          <div className="alert_item">
            <div className="text d-block w-100">
                          ...
                          <div className="change-avatar">
                            ...
                            <div className="upload-img">
                              ...
                              <small className="form-text text-muted">
                                Format: JPG, GIF yoki PNG. Hajmi 5MB dan
                                oshmasin!{' '}
                              </small>
                            </div>
                          </div>
